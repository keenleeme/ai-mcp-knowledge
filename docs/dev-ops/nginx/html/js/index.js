...
function appendMessage(content, isAssistant = false, saveToStorage = true) {
    ...
    const visibleContent = filterThinkBlocks(content);
    const renderedContent = DOMPurify.sanitize(marked.parse(visibleContent));
    ...
    if (saveToStorage && currentChatId) {
        ...
        chatData.messages.push({ content: visibleContent, isAssistant });
    ...
    currentEventSource.onmessage = function(event) {
        try {
            ...
            if (data.result?.output?.content) {
                ...
                const visible = filterThinkBlocks(accumulatedContent);
                ...
                const rendered = DOMPurify.sanitize(marked.parse(visible || ''));
                tempMessageDiv.innerHTML = rendered;
                chatArea.scrollTop = chatArea.scrollHeight;
            }
            ...
            if (data.result?.output?.properties?.finishReason === 'STOP') {
                ...
                const finalContentRaw = accumulatedContent;
                const finalContent = filterThinkBlocks(finalContentRaw);
...
const openGitModalBtn = document.getElementById('openGitModal');
...
const gitModalClose = document.getElementById('gitModalClose');
const gitForm = document.getElementById('gitForm');
const gitRepoInput = document.getElementById('gitRepoUrl');
const gitUserInput = document.getElementById('gitUserName');
const gitTokenInput = document.getElementById('gitToken');
const gitLoadingOverlay = document.getElementById('gitLoadingOverlay');
const gitAlert = document.getElementById('gitAlert');
...
function closeGitModal() {
    ...
    if (gitUserInput) gitUserInput.value = '';
    if (gitTokenInput) gitTokenInput.value = '';
    if (gitAlert) gitAlert.classList.add('hidden');
}
...
if (gitModalClose) {
    gitModalClose.addEventListener('click', () => {
        closeGitModal();
    });
}

if (uploadFileInput && uploadFileList) {
    uploadFileInput.addEventListener('change', function(e) {
        ...
        files.forEach((file, index) => {
            ...
            li.innerHTML = `<span>${file.name}</span>
                <button type="button" class="text-red-500 hover:text-red-700" data-index="${index}">删除</button>`;
    ...
    uploadFileList.addEventListener('click', function(e) {
        ...
        const items = Array.from(uploadFileList.children);
        if (items[index]) {
            items[index].remove();
        }
    });
...
if (uploadForm) {
    uploadForm.addEventListener('submit', function(e) {
        e.preventDefault();
        if (!uploadFileInput || !uploadTitleInput) return;
        ...
        formData.append('ragTag', uploadTitleInput.value);
        files.forEach(file => formData.append('file', file));

        fetch('http://localhost:8090/api/v1/rag/file/upload', {
            method: 'POST',
            body: formData
        })
            .then(res => {
            ...
            .then(data => {
                const ok = data && (data.code === '0000' || data.code === '0');
                if (!ok) {
                    throw new Error(data.info || '上传失败');
                }
                if (uploadAlert) {
                    uploadAlert.textContent = data.info || '上传成功';
                    uploadAlert.className = 'absolute top-10 left-1/2 -translate-x-1/2 px-4 py-2 rounded-md text-sm shadow bg-emerald-100 text-emerald-700';
            ...
            .catch(err => {
                if (uploadAlert) {
                    uploadAlert.textContent = err.message || '上传失败';
                    uploadAlert.className = 'absolute top-10 left-1/2 -translate-x-1/2 px-4 py-2 rounded-md text-sm shadow bg-rose-100 text-rose-700';
                    uploadAlert.classList.remove('hidden');
                } else {
...
if (gitForm) {
    gitForm.addEventListener('submit', function(e) {
        e.preventDefault();
        if (!gitRepoInput || !gitUserInput || !gitTokenInput) return;

        const repoUrl = gitRepoInput.value.trim();
        const userName = gitUserInput.value.trim();
        const token = gitTokenInput.value.trim();

        if (!repoUrl || !userName || !token) {
            alert('请完整填写仓库地址、用户名和密码/Token');
            return;
        }

        if (gitLoadingOverlay) {
            gitLoadingOverlay.classList.remove('hidden');
        }
        ...
        fetch('http://localhost:8090/api/v1/rag/analyze_git_repository', {
            ...
            .then(data => {
                const ok = data && (data.code === '0000' || data.code === '0');
                if (!ok) {
                    throw new Error(data.info || '仓库上传失败');
                }
                if (gitAlert) {
                    gitAlert.textContent = data.info || '仓库上传成功';
                    gitAlert.className = 'absolute top-10 left-1/2 -translate-x-1/2 px-4 py-2 rounded-md text-sm shadow bg-emerald-100 text-emerald-700';
                    gitAlert.classList.remove('hidden');
                }
                setTimeout(() => {
                    closeGitModal();
                    loadRagOptions();
            ...
            .catch(err => {
                if (gitAlert) {
                    gitAlert.textContent = err.message || '仓库上传失败';
                    gitAlert.className = 'absolute top-10 left-1/2 -translate-x-1/2 px-4 py-2 rounded-md text-sm shadow bg-rose-100 text-rose-700';
                    gitAlert.classList.remove('hidden');
                } else {
                    alert(err.message || '仓库上传失败');
                }
            })
            .finally(() => {
                if (gitLoadingOverlay) {
                    gitLoadingOverlay.classList.add('hidden');
                }
            });
    });
}
